...
# Sample the data
set.seed(4321)
smpl <- c(sample(linesBlogs, length(linesBlogs) * 0.002),
          sample(linesNews, length(linesNews) * 0.002),
          sample(linesTwitter, length(linesTwitter) * 0.002))

# Load the data as a corpus
corp <- Corpus(VectorSource(smpl))
...
stcorp <- tm_map(corp, removeWords, stopwords("english"))
...
# calculate frequency
f1w <- getFrequency(removeSparseTerms(TermDocumentMatrix(stcorp), 0.99))
